#!/bin/bash
#################################################################################################################
# copy_to_remote.sh  copy files to remote server.
#
# Dependencies: sshpass,scp
# Platform: linux
#
# USAGE: sh copy_to_remote.sh [-h] [-s] [-H remote_ip] [-P port] [-u user_name]
#                               [-p password] [-r remote_file_path] [-l local_file_path]
# Run "sh copy_to_remote.sh -h" for detail.
#
...
{
    echo "Usage: sh copy_to_remote.sh [OPTION...]
...
USAGE: sh copy_to_remote.sh [-h] [-s] [-H remote_ip] [-P port] [-u user_name]
  ...
  -r remote_file_path      Copy file to remote server path
  -l local_file_path       File's path on local server which want to be copied.
...
function check_dependency
{
    which $1 > /dev/null
    if [[ $? -ne 0 ]];then
        echo "copy_to_remote.sh depends on $1, please install it."
        exit -1
    fi
}
...
say "[START] $(date '+%Y%m%d %H%M%S') $command"
...
if [[ $? -ne 0 ]];then
    ...
    say "[FINISHED] $(date '+%Y%m%d %H%M%S') $command"
fi
